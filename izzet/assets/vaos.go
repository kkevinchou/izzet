...
func (m *AssetManager) CreateBatch(meshHandles []types.MeshHandle, modelMatrices []mgl32.Mat4) (uint32, int32) {
	...
	var uniqueVertexCount int
	var vertexIndices []uint32
	for i, meshHandle := range meshHandles {
		p := m.GetPrimitives(meshHandle)[0].Primitive
		...
		vertexIndexOffset := uniqueVertexCount
		for _, index := range p.VertexIndices {
			vertexIndices = append(vertexIndices, index+uint32(vertexIndexOffset))
		}

		uniqueVertexCount += len(p.UniqueVertices)
	}

	totalAttributeSize := len(vertexAttributes) / uniqueVertexCount
	...
	return vao, int32(len(vertexIndices))
}
