...
func (r *Renderer) drawTranslationGizmo(viewerContext *ViewerContext, shader *shaders.ShaderProgram, position mgl32.Vec3) {
	colors := map[int]mgl32.Vec3{
		gizmo.GizmoXAxisPickingID: mgl32.Vec3{1, 0, 0},
		gizmo.GizmoYAxisPickingID: mgl32.Vec3{0, 0, 1},
		gizmo.GizmoZAxisPickingID: mgl32.Vec3{0, 1, 0},
	}
	...
	nearPlanePosition := NDCToWorldPosition(*viewerContext, mgl32.Vec3{screenPosition.X(), screenPosition.Y(), -float32(r.app.RuntimeConfig().Near)})
	renderPosition := nearPlanePosition.Sub(viewerContext.Position).Mul(settings.GizmoDistanceFactor).Add(nearPlanePosition)

	shader.Use()
	shader.SetUniformMat4("model", mgl32.Translate3D(renderPosition.X(), renderPosition.Y(), renderPosition.Z()))
	...
	for _, entityID := range axisEntityIDs {
		...
		lines := [][2]mgl32.Vec3{{mgl32.Vec3{}, axis.Direction}}
...
func (r *Renderer) drawScaleGizmo(viewerContext *ViewerContext, shader *shaders.ShaderProgram, position mgl32.Vec3) {
	screenPosition, behind := worldToNDCPosition(*viewerContext, position)
	if behind {
		return
	}
	...
	colors := map[int]mgl32.Vec3{
		...
		gizmo.GizmoYAxisPickingID:   mgl32.Vec3{0, 0, 1},
		gizmo.GizmoZAxisPickingID:   mgl32.Vec3{0, 1, 0},
		gizmo.GizmoAllAxisPickingID: mgl32.Vec3{1, 1, 1},
	}
	hoverColor := mgl32.Vec3{1, 1, 0}
...
func (r *Renderer) drawCircleGizmo(viewerContext *ViewerContext, position mgl32.Vec3, renderContext RenderContext) {
	screenPosition, behind := worldToNDCPosition(*viewerContext, position)
	if behind {
		return
	}
...
func NDCToWorldPosition(viewerContext ViewerContext, directionVec mgl32.Vec3) mgl32.Vec3 {
	// ndcP := mgl32.Vec4{((x / float32(g.width)) - 0.5) * 2, ((y / float32(g.height)) - 0.5) * -2, -1, 1}
...
func worldToNDCPosition(viewerContext ViewerContext, worldPosition mgl32.Vec3) (mgl32.Vec2, bool) {
